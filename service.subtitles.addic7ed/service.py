...
def get_subs(url, langs, filename_string, cont=None):
    if cont:
        ...
        content = get_url(url)
    sublinks = []
    regex = '''>Version (.+?),.+?uploaded.+?<a.+?>(.+?)<.+?class="language">(.+?)<.+?Download" href="(.+?)"'''
    match = re.compile(regex, re.IGNORECASE | re.MULTILINE | re.DOTALL).findall(content)
    for ver, uploader, language, rawurl in match:
        lang = get_language_info(language)
        if lang['3let'] in langs:
            link = "%s%s" % (self_host, rawurl)
            sublinks.append({'rating': '0', 'filename': "%s-%s-%s" % (filename_string, ver, uploader), 'sync': '', 'link': link, 'lang': lang, 'hearing_imp': ''})
    return sublinks
...
def Search(item):
    ...
    if len(item['tvshow']) > 0:
        ...
        if str(item['year']) == "":
            ...
            item['title'] = addic7ize((re.sub('S(\d{1,2})E(\d{1,2})', '', item['title'])).strip())
