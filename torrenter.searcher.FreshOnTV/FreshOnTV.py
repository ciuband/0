...
class FreshOnTV(SearcherABC.SearcherABC):
    ...
    headers = [('Origin', 'https://' + baseurl),
        ...
        ('Referer', 'https://' + baseurl + '/'), ('X-Requested-With', 'XMLHttpRequest')]
    ...
    def search(self, keyword):
        filesList = []
        url = 'https://%s/browse.php?search=%s' % (self.baseurl, urllib.quote_plus(keyword))
        if self.__settings__.getSetting('sortby') == '0':
            url += '&sort=seeders&h=10&d=DESC' 
	elif self.__settings__.getSetting('sortby') == '1':
            url += '&sort=added&h=6&d=DESC'
	elif self.__settings__.getSetting('sortby') == '2':
            url += '&sort=leechers&h=11&d=DESC'
        response = self.makeRequest(url, headers=self.headers)
        if None != response and 0 < len(response):
            self.debug(response)
            regex = '''<tr class="torrent(.+?)</tr>'''
            regex_tr = '''title=".+?">(.+?)<.+?a href="/download.php(.+?)".+?size">(.+?)</td>.+?class=".+?>(.+?)<.+?class="c.+?>.+?(\d+).+?<.+?class="l.+?>.+?(\d+).+?<'''
            for tr in re.compile(regex, re.DOTALL).findall(response):
	        if re.compile('<title> FileList :: Login </title>').search(response):
                    xbmc.executebuiltin((u'Notification(%s,%s)' % ('FreshOnTV', 'lipsa username si parola din setari')))
                result = re.compile(regex_tr, re.DOTALL).findall(tr)
                self.debug(tr + ' -> ' + str(result))
                if result:
                    (title, link, size, nothing, seeds, leechers) = result[0]
                    title = self.clear_title(title)
                    link = 'https://%s/download.php%s' % (self.baseurl, link)
                    filesList.append((
                                     int(int(self.sourceWeight) * int(seeds)),
                                     int(seeds), int(leechers), self.clear_title(size),
                                     title,
                                     self.__class__.__name__ + '::' + link,
                                     self.searchIcon,
                                     ))
        #with open('/storage/.kodi/temp/files.py', 'wb') as f: f.write(repr(result))
        return filesList
    ...
    def getTorrentFile(self, url):
        ...
        if re.search("<html", content):
            msg = re.search('Username or password is incorrect', content)
            if msg:
                self.showMessage('FreshOnTV Login Error', 'Incorrent pair login/password')
            xbmc.sleep(4000)
    ...
    def login(self):
        ...
        x = self.makeRequest(
                             'https://%s/login.php?action=makelogin' % (self.baseurl), data=data, headers=self.headers)
        if re.search('href="browse.php"', x):
            self.log('LOGGED FreshOnTV')
        self.cookieJar.save(ignore_discard=True)
        for cookie in self.cookieJar:
            if cookie.name == 'pass' and cookie.domain == self.baseurl:
                return 'pass=' + cookie.value
        return False
